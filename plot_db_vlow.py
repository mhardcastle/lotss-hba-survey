...
with SurveysDB(readonly=True) as sdb:
    sdb.cur.execute('select * from fields where dr2 or vlow_image is not NULL')
...
_,r=plot_select(results,lambda r:r['vlow_image'] in ['Complete','Archived'],label='Complete',color='green')
_,r=plot_select(r,lambda r:r['vlow_image'] in ['Failed','Download failed', 'WSCLEAN failed'],label='Failed',color='red')
_,r=plot_select(r,lambda r:r['vlow_image'] in ['Running','WSCLEAN','WSCLEAN complete','Subtracting','Subtracted'],label='Running',color='cyan')
_,r=plot_select(r,lambda r:r['vlow_image'] in ['Downloading','Uploading'],label='In progress',color='yellow')
