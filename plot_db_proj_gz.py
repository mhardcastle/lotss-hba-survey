...
def plot_select(r,sf,label,**kwargs):
    ...
    for f in r:
        if sf(f):
            ra.append(f['ra'])
            dec.append(f['decl'])
            if f['gz_status']!="Complete" and f['gz_status']!="In progress":
                alphas.append(0.3)
            else:
                alphas.append(1.0)
            r_in.append(f)
    ...
    start=True
    for ra,dec,alpha in zip(ra_r,dec_r,alphas):
        if start:
            plt.scatter(ra,dec,label=label,alpha=alpha,**kwargs)
            start=False
        else:
            plt.scatter(ra,dec,alpha=alpha,**kwargs)
    print "%-20s : %i" % (label,len(r_in))
    return r_in,r_out
    
with SurveysDB(readonly=True) as sdb:
    sdb.cur.execute('select fields.id as id,ra,decl,gz_status,fields.status as status,observations.status as ostatus,observations.location as location,sum(nsb*integration/232) as s,count(observations.id) as c,fields.priority from fields left join observations on (observations.field=fields.id) group by fields.id having ostatus is not null and ostatus!="Scheduled"')
...
_,r=plot_select(r,lambda r:True,label='Observed',color='grey',s=5)
...
plt.legend(loc=4)
plt.tight_layout()
plt.title('DR2 Galaxy Zoo status at '+datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"),loc='right')
if len(sys.argv)>1:
