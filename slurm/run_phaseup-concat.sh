...
singularity exec -B ${PWD},${BINDPATHS} ${SIMG} python3 ${FLOCSDIR}/runners/create_ms_list.py VLBI concatenate-flag  --delay_calibrator ${DATADIR}/delay_calibrators.csv --selfcal ${FACETSELFCAL} --h5merger ${LOFARHELPERS} --configfile ${VLBIDIR}/facetselfcal_config.txt  --linc ${LINCDIR} ${DATADIR}/ >> test.log 2>&1
