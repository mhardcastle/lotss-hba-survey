...
singularity exec -B ${PWD},${BINDPATHS} ${LOFAR_SINGULARITY} python3 ${FLOCSDIR}/runners/create_ms_list.py VLBI concatenate-flag  --delay_calibrator ${DATADIR}/delay_calibrators.csv --selfcal ${FACETSELFCAL} --h5merger ${LOFARHELPERS} --configfile ${VLBIDIR}/facetselfcal_config.txt  --linc ${LINCDIR} ${DATADIR}/ >> test.log 2>&1


echo LINC starting
echo export PYTHONPATH=\$LINC_DATA_ROOT/scripts:\$PYTHONPATH > tmprunner_${OBSID}.sh
echo 'cwltool --parallel --preserve-entire-environment --no-container --tmpdir-prefix=${TMPDIR} --outdir=${OUTDIR} --log-dir=${LOGSDIR} ${VLBIDIR}/workflows/phaseup-concat.cwl mslist_VLBI_phaseup-concat.json' >> tmprunner_${OBSID}.sh
