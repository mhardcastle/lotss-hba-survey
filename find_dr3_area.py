...
for archived in [False,True]:
    ...
    if archived:
        ...
        command='select ra,decl from fields where status="Verified"'
    else:
        print('Doing full sky area')
        command='select ra,decl from fields'
    ...
    for i in range(len(ra)):
        pixels=hp.cone_search_lonlat(ra[i]*u.deg,dec[i]*u.deg,2.0*u.deg)
