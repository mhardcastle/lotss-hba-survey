...
with SurveysDB(readonly=True) as sdb:
    ...
    sdb.cur.execute('select fields.id as id,ra,decl,dr2,dr3,lotss_field,fields.status as status,observations.status as ostatus,observations.location as location,sum(nsb*integration/232) as s,count(observations.id) as c,fields.priority,required_integration from fields left join observations on (observations.field=fields.id) and (observations.status="Archived" or observations.status="DI_Processed") group by fields.id having lotss_field=1')
...
plt.legend(loc=4)
plt.tight_layout()
plt.title('Mosaic processing status at '+datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"),loc='right')
if len(sys.argv)>1:
